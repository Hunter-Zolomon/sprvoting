...
@WebFilter(filterName = "PartyFilter", urlPatterns = {"/party/*"})
public class PartyFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        if (HelperFunctions.isSessionValid(session) && HelperFunctions.isUser(session, "Party")) {
            chain.doFilter(request, response);
        } else if (HelperFunctions.isSessionValid(session) && !HelperFunctions.isUser(session, "Party")) {
